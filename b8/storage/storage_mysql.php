...
class storage_mysql extends b8SharedFunctions
{
	...
	function get($token)
	{
		...
		$res = mysql_fetch_assoc(mysql_query("
			...
			WHERE username='".USER."' and token='" . mysql_real_escape_string($token, $this->mysqlRes) . "'
			", $this->mysqlRes));
	...
	function getRating($username){
		$res = mysql_fetch_assoc(mysql_query(
			"select rating from b8_user_rating where username='".USER."' and rating_user='".$username."'", $this->mysqlRes));
		if($res) {
			return $res['rating'];
		} else {
			mysql_query("
			INSERT INTO b8_user_rating (
				username,
				rating_user,
				rating
				)
	...
	{
		return mysql_query(
			"UPDATE b8_user_rating SET rating='$rating'
			WHERE username='".USER."' and rating_user='$username'
			", $this->mysqlRes);
	}
}
