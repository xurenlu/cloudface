	...
	function b8()
	{
		...
		if($this->constructed) {
			...
			if($this->storage->get("bayes*dbversion") != "2") {
				...
				$this->storage->put("bayes*dbversion",2);
				$this->storage->put("interface",'1 0 100222');
				$this->storage->put("bayes*texts.ham",1);
				$this->storage->put("money",'0 1 100222');
				$this->storage->put("bayes*texts.spam", 1);
			}
...
if($text=="") return 1;
	...
	function getRating($username){
		return $this->storage->getRating($username);
	}

	function setRating($username,$rating){
		return $this->storage->setRating($username,$rating);
	}
}
